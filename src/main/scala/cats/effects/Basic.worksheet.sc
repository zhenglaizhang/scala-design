...
def inputStream(f: File, guard: Semaphore[IO]): Resource[IO, FileInputStream] =
  Resource.make {
    ...
    // guard.withPermit {}
    // guard.permit.
    guard.acquire >> IO(inStream.close()).handleErrorWith(_ =>
      IO.unit
    ) >> guard.release
  }
...
def outputStream(
    f: File,
    guard: Semaphore[IO]
): Resource[IO, FileOutputStream] =
  Resource.make {
    IO(new FileOutputStream(f))
  } { outStream =>
    guard.acquire >> IO(outStream.close()).handleErrorWith(_ =>
      IO.unit
    ) >> guard.release
  }
...
def copy(origin: File, destination: File)(implicit
  ...
  for {
    ...
    count <- inputOutputStreams(origin, destination, guard).use {
      case (in, out) => guard.acquire >> transfer(in, out)
    } // todo guard.withPermit ???
  } yield count
}
  ...
  (inIO, outIO).tupled
    .bracket {
      ...
      case (
            in,
            out
          ) => // When using bracket, if there is a problem when getting resources in the first stage, then the release stage will not be run
